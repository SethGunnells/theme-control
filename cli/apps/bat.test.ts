...
import { resolveConfig, installThemes, updateIfEnabled } from "./bat";
import { createTestContext } from "../tests/utils";

// Import theme files for content verification using Bun.file
const rosePineThemeFile = Bun.file(
  new URL("../bat-themes/rose-pine.tmTheme", import.meta.url),
);
const rosePineDawnThemeFile = Bun.file(
  new URL("../bat-themes/rose-pine-dawn.tmTheme", import.meta.url),
);
...
describe("bat module", () => {
  ...
  describe("installThemes", () => {
    test("should create themes directory if it doesn't exist", async () => {
      ...
      await installThemes(TEST_THEMES_PATH, false);
    ...
    test("should update theme files when forceUpdate is true", async () => {
      ...
      await installThemes(TEST_THEMES_PATH, true);
